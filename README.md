...
默认进行自然语言多轮对话；按需切换到 Agent 任务，执行 Task Brief、计划、暂停、
继续、追加指引、受控工具、验证和报告；刷新后恢复消息与运行状态。
...
1. 点击“新建对话”。默认回复方式为“对话”，适合问候、解释和多轮讨论。
2. 普通对话直接返回自然语言，不创建 Run，也不要求成功正则或结构化 JSON。
3. 需要计划、工具和验证时，在输入区切换到“Agent 任务”，再填写任务设置并发送。
4. Agent 任务可在安全检查点暂停和继续；“追加指引”按序排队，应用后会明确显示
   “已应用并重规划”。停止仍表示终止当前 Run，与暂停不同。
5. 结果卡先给简洁结论，完整事件、工具参数、证据、预算和报告放在可打开的运行审计中。
...
- **Thread（对话）**：保存消息、附件、回复方式和所选 Agent，刷新页面后仍可继续。
...
普通对话通过独立聊天 API 调用 Provider，只保存完整用户/助手消息。只有明确选择
Agent 任务时才创建 Run，进入五阶段流程并通过 SSE 推送公开事件。

## 对话与 Agent 任务怎么选

- **对话**：默认方式。适合问候、解释代码、讨论方案和连续追问，不创建 Run。
- **Agent 任务**：适合需要 Task Brief、计划确认、工具、验证、暂停/继续和审计的工作。
- **普通/竞赛限制**：只约束 Agent 执行，不是“对话/Agent 任务”的替代名称。
- **建议/结构化/证据完成模式**：属于 AgentProfile；证据模式需要确定性验证表达式。

第一次体验先使用“对话”。需要执行与验证时再切换到“Agent 任务”，并查看
