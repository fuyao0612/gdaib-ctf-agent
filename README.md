...
启动器会读取 `.env` 校验主密钥配置，但不会显示或输出密钥。若 Docker Hub 因网络或 HTTPS 代理不可用而无法构建镜像，启动器会优先打开已有本地开发服务，或在本机依赖已安装时自动降级为本地开发模式（`127.0.0.1:5173`）。设置中心会自动建立仅限本机的安全会话；关闭这个窗口不会停止服务，停止仍运行 `.\yuwang.ps1 stop`，本地模式使用 `.\yuwang.ps1 stop -Development`。
...
同一条消息会依照当前任务状态得到不同处理，用户不需要选择模式。每次发送
...
| 当前状态 | 同一输入框的处理 |
| --- | --- |
| 没有活动任务 | 保存来源消息、固化快照并创建受控 Agent Run。 |
