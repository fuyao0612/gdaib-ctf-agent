...
御网智元是一个单机自托管、可审计的任务型安全 Agent 工作台。它把用户任务、
模型调用、受控工具、验证结果、预算和报告保存在本地 SQLite 中，适合计算机专业
学生学习“一个 Agent 如何从请求走到可验证结果”。

当前能做：连接 DeepSeek、阿里云百炼/千问、智谱 GLM 或其他 OpenAI 兼容 API；
使用一个统一输入框提交任务；没有活动任务时，每条消息都会创建受控 Run，不再按关键词分派普通聊天、
执行或澄清模式。Agent 在同一个循环内决定是否调用已授权工具或请求补充。任务运行、等待补充或暂停时，输入框仍可
...
3. 确认页面显示“Provider 已连接”“默认 Agent 可用”“可以开始任务”。
4. 保存 Agent 默认设置，关闭设置，创建任务并提交任务描述。
...
1. 点击“新建任务”，填写可选的任务名称后创建。
2. 在同一个输入框中提交“你好”、解释代码、讨论方案或“分析这个文件并给出结果”。每条输入都会进入
   同一个受控 Agent Run；是否调用工具由 Agent 根据授权、策略和任务本身决定。无需工具的结果会明确标记为“未经外部验证”。
3. Provider、任务配置和验证规则由设置中心的默认配置决定；附件始终作为不可信上下文。
...
6. 结果卡先给简洁结论；计划、工具参数、证据、预算和完整报告默认折叠，可在“任务详情与控制”
   或“运行审计”中查看。审计会显示实际模型、开始结束时间、Token/费用来源和人工介入次数。
