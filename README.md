...
使用一个统一输入框完成自然语言多轮对话和受控任务；没有活动任务时，服务端通过保守的确定性规则
识别明确执行请求、模糊请求和普通聊天，不额外发起意图分类模型调用。明确任务进入同一个 Agent 循环后，
才由模型决定是否调用已授权工具。任务运行、等待补充或暂停时，输入框仍可
在当前任务允许的范围内继续发送消息、附件或停止指令；刷新后从本地 SQLite 恢复消息和运行状态。
...
打开 <http://127.0.0.1:8080>。以后只需记住 `yuwang.ps1`；查看全部命令：
...
启动并等待引擎就绪。双击入口每次都会使用 Docker 构建缓存检查并重建当前源码，因此源码
更新后不会复用旧的 Web 静态文件；首次启动或有更新时可能需要几分钟。健康检查通过后会按
`.env` 中实际的 `YUWANG_WEB_PORT` 打开浏览器，并统一使用 `127.0.0.1` 避开少数 Windows
环境中 `localhost` 优先解析到不可达 IPv6 地址的问题。构建或配置失败时窗口会保留并显示原因。
启动器会读取 `.env` 校验主密钥配置，但不会显示或输出密钥。设置中心会自动建立仅限本机的安全会话；关闭这个窗口不会停止服务，停止仍运行 `.\yuwang.ps1 stop`。
...
1. Web 设置中心默认进入“新手模式”，会自动建立本机会话。依次选择厂商、核对 API 地址、填写 API Key
   和模型，然后保存 Provider（模型服务连接配置）。
2. 点击“连接测试”。这是一次真实模型请求，不是模拟成功。
3. 确认页面显示“Provider 已连接”“默认 Agent 可用”“可以开始对话”。
4. 保存默认聊天模型，关闭设置，创建对话并发送消息。

新手模式和高级模式操作同一份正式配置。高级模式可继续修改超时、重试、费用、
备用 Provider、结构化输出、Agent 预算、上下文、记忆、规划、验证、Skills 和版本。

Skills 是仅含说明、提示词、步骤和检查清单的声明式任务模板，不支持上传或执行代码。可在高级设置
创建或停用，在输入框附近勾选后只影响下一次新 Run；创建 Run 时内容会固化为快照，之后修改或删除
设置不会改变历史运行。
...
2. 在同一个输入框中发送“你好”、解释代码或讨论方案，系统会直接流式返回自然语言，
   不创建 Run，也不要求成功正则或结构化 JSON。附件始终作为不可信上下文；上传附件后
   发送“分析这个文件并给出结果”等明确分析请求，会进入受控执行。
3. 发送“完成这道授权 CTF 题”“执行任务”或“验证并报告结果”等明确受控执行请求时，
   系统自动开始受控任务，并显示简短进度；Provider、任务配置和验证规则由设置中心的默认配置决定。
   “请完成这道 Web 题并找到 flag”“分析这个附件并给出结果”“检查本地靶场并给出验证结果”
   也属于这类请求。若表达中明确写有“只解释，不要执行”，则保持直接回复。
4. 受控任务可在安全检查点暂停和继续：在“任务详情与控制”点击“安全暂停”，到达检查点后
   再点击“从检查点继续”。运行中发送的内容会按序作为追加指引保存；等待补充时，同一输入框的
   内容会作为补充并从检查点继续；暂停时，指引会先保存、恢复后再在安全检查点应用。运行中发送
   “停止”“取消”或“终止”会提交停止请求；若任务仍在执行，界面会显示“停止请求处理中”并继续
   接收状态，直到更新为“已停止”，这与暂停不同。
5. 低风险的已注册工具可按既有授权自动执行；中风险工具会显示具体名称，必须点击“确认并执行”
   后才会调用，高风险工具默认拒绝。一次确认只绑定当前工具和参数，后续变化会再次请求确认。
6. 结果卡先给简洁结论；计划、工具参数、证据、预算和完整报告默认折叠，可在“任务详情与控制”
   或“运行审计”中查看。审计会显示模型、开始结束时间、Token/费用来源和人工介入次数；普通聊天
   不会显示空审计面板。
...
- **Task Brief（任务说明）**：由 Agent 生成的公开任务摘要，包含目标、授权范围、限制、成功标准和必要澄清；不展示隐藏思维链。
...
Web 始终调用统一消息入口。没有活动任务时，后端优先识别“只解释，不要执行”等否定表达：
普通问题和知识解释走自由文本流，明确的执行、完成题目、附件分析、调用能力或验证结果请求
才开始受控任务。`interaction_mode` 字段仍保留在历史数据和兼容 API 中，但不再是用户需要选择的
概念。

## 统一消息生命周期

同一条消息会依照当前对话状态得到不同处理，用户不需要切换“聊天/任务”模式。每次发送
都带有请求 ID；重试或网络重连重复提交相同请求时，后端会复用已有的指引、补充或消息记录，
避免在时间线中重复写入。回复失败或连接中断时，输入文字和待发送附件会保留；点击“重试回复”
会复用原请求 ID。切换或新建对话会清空这份草稿，旧会话的迟到响应不会写入新会话。

| 当前状态 | 同一输入框的处理 |
| --- | --- |
| 没有活动任务 | 服务端判断为直接回复，或开始受控任务。 |
| 排队中、运行中、等待计划确认 | 保存为时间线中的用户消息，并按当前任务内序号排队为追加指引；不会自动批准计划。 |
| 等待补充 | 保存为补充内容，从持久化检查点继续。 |
| 等待任务澄清 | 保存为澄清内容，从持久化检查点继续处理。 |
| 已暂停 | 保存为追加指引；点击“从检查点继续”后在安全节点应用。 |
| 有活动任务且输入停止短语 | 提交停止请求，不新建任务；执行中会先显示“停止请求处理中”。 |

上传附件使用的也是这一个输入区。附件必须属于当前对话，且会随消息作为不可信上下文
记录；它不能改变既有授权范围。在限制补充信息的任务模式中，服务端会拒绝不符合该边界的
提交并提示原因。运行控制面板会逐条显示追加指引的序号、提交时间和状态：`已排队`、
`已在检查点应用`，或`已在检查点应用，因本指引重规划`。若任务与指引同时进入最后收尾，
已成功入库但来不及经过安全检查点的记录会明确显示“任务已结束，未应用”，保留审计但不会
伪装成已应用或长期停留在“已排队”。

## 系统会如何处理我的消息？

- **直接回复**：问候、解释代码、讨论方案和连续追问，都会流式返回自然语言。
- **受控执行**：明确要求执行任务、调用工具、完成授权 CTF、验证并报告时，自动进入任务准备、
  计划、工具、验证和报告流程；带附件的明确分析、检查、提取或验证请求也会走这一路径。
- **停止**：运行期间发送“停止”“停止生成”“停止任务”“取消”或“终止”，会向当前 Run 提交
  停止请求；正在执行时会先显示“停止请求处理中”并继续接收状态，终态才显示“已停止”。也可以
  使用输入框右侧的停止按钮。
...
- **排队中 / 运行中**：任务正在等待或执行；可以继续发送追加指引，暂停会在安全检查点生效。
- **等待任务澄清**：阅读“任务说明”中的问题，在统一输入框补充信息。
- **等待计划确认**：检查计划范围、步骤和验证方式，再批准或提出修改。
- **暂停已排队 / 任务已暂停**：前者正在等待安全检查点，后者已经保存状态、计划和指引，可选择“从检查点继续”。
- **成功**：运行已完成；结果卡还会说明是否经过验证，二者不是同一个概念。
- **失败**：预算、模型、工具或验证明确失败；先阅读失败原因和建议的下一步。
- **已停止**：用户主动停止或系统安全边界终止；已有事件和证据仍会保留。
- **等待用户**：Agent 缺少必要信息；在下方统一输入框回答后可从检查点继续，不必重新建任务。

结果“已完成”不等于“已验证”。没有配置确定性外部验证条件时，任务仍可以给出结论，
但结果卡会显示“回答已完成（未外部验证）”；结构化输出只通过 JSON Schema 校验时会显示
“回答已完成（部分验证）”；规则不匹配、来源不一致或结构校验失败会明确显示“验证失败”。只有候选值同时满足具体规则、与真实成功的
受控工具证据绑定且来源值一致时，才显示“已验证成功”；模型自称完成、普通非空文本以及
`.+`、`.*` 一类万能正则都不能升级为已验证。

较早对话会生成带范围的滚动摘要，最新补充或纠偏单独保留并优先于旧摘要。小型文本附件以不可信内容受限进入上下文；大型附件和超长工具输出会保留在本地 Artifact，只将摘要和引用放入任务上下文。暂停、刷新页面或服务重启后可继续使用同一份检查点、摘要和 Artifact 引用。
...
apps/api/
  ...
  routes/messages.py              唯一消息入口与状态分派
  run_interactions.py             指引、补充、澄清的幂等持久化与恢复
...
- 20～25 分钟：阅读 `apps/api/routes/messages.py`、`apps/api/run_interactions.py` 与
  `apps/api/context.py`，找到统一消息、Run 快照和后台调度入口。
...
开发地址为 Web <http://127.0.0.1:5173>、API <http://127.0.0.1:8000>、OpenAPI
<http://127.0.0.1:8000/api/docs>。开发数据在 `data/development/`，日志在
...
脚本会自动建立本机安全会话。Provider 尚未真实测试成功时，它会直接给出中文配置提示，不会伪造结果。
