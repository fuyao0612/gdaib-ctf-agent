...
def test_ctf_report_renders_persisted_timeline_without_duplicate_h1() -> None:
    ...
    steps = [
        {
            ...
            "goal": "读取首页",
            "action_summary": "请求首页",
            "observation_summary": "HTTP 200；页面显式链接：/robots.txt",
            "decision": "下一步：读取 robots.txt",
        },
        {
            "sequence": 2,
            "call_id": str(uuid4()),
            "goal": "读取 robots",
            "action_summary": "请求 robots.txt",
            "observation_summary": "robots.txt 暴露路径：/dev-notes.txt",
            "decision": "下一步：读取开发说明",
        },
        {
            "sequence": 3,
            "call_id": str(uuid4()),
            "goal": "读取说明",
            "action_summary": "请求 /dev-notes.txt",
            "observation_summary": "开发说明给出 /api/debug 的访问条件",
            "decision": "下一步：请求调试接口",
        },
        {
            "sequence": 4,
            "call_id": str(uuid4()),
            "goal": "读取调试接口",
            "action_summary": "请求 /api/debug",
            "tool_id": "builtin.localhost_http_probe",
            "arguments": {
                "url": "http://127.0.0.1:8088/api/debug?unlock=1",
                "ctf_header": {"name": "X-CTF-Token", "value": "sunrise-7"},
            },
            "observation_summary": "响应正文包含 flag_b64 字段",
            "decision": "下一步：使用 Base64 解码",
        },
        {
            "sequence": 5,
            "call_id": str(uuid4()),
            "goal": "解码",
            "action_summary": "执行 Base64 解码",
            "tool_id": "ctf.encoding_decode",
            "arguments": {"text": "ZmxhZ3tkZW1vfQ==", "encoding": "base64"},
            "observation_summary": "解码得到 1 个高置信 Flag 候选",
            "decision": "下一步：校验 Flag 格式",
        },
        {
            "sequence": 6,
            "call_id": str(uuid4()),
            "goal": "校验候选",
            "action_summary": "检查 Flag 候选格式",
            "observation_summary": "候选 Flag flag{demo}；格式校验状态：format_matched；尚未经过赛题平台验证",
            "decision": "结束：已完成格式校验",
        },
    ]
    ...
    assert (
        sum(line.startswith("# ") and not line.startswith("## ") for line in markdown.splitlines())
        == 1
    )
...
def test_report_uses_persisted_run_result_instead_of_reconstructing_it() -> None:
    ...
    run.results.append(persisted)
    _, data = ReportGenerator().generate(
        run,
        TaskSpec(body="提取 IOC", scenario="incident_response"),
        [],
        {"trace": {"steps": [], "metrics": {}, "artifacts": []}},
    )
    assert data["task_result"] == persisted.model_dump(mode="json")
...
def test_report_keeps_custom_prefix_candidate_and_deduplicates_sources() -> None:
    ...
    _, data = ReportGenerator().generate(
        ...
        [],
        {
                ...
                "steps": [
                    {
                        ...
                        "call_id": call_id,
                        "tool_id": "ctf.encoding_decode",
                        "observation_summary": "解码完成",
                        "preview": "GDAIB{custom_prefix}",
                    }
                ],
            ...
            "evidence_records": [
                {
                    "candidate": "GDAIB{custom_prefix}",
                    "source_call_id": call_id,
                    "source_step": 1,
                    "location": "/candidates/0/value",
                    "discovery_source": "encoding_decode",
...
def test_historical_ctf_trace_recovers_legacy_artifacts_and_uses_real_request_parameters(
    tmp_path,
) -> None:
    ...
    steps = [
        ...
        (
            "http://127.0.0.1:8088/dev-notes.txt",
            {"name": "X-CTF-Build-Token", "value": "sunrise-7"},
            "开发说明给出 /api/debug",
        ),
        (
            "http://127.0.0.1:8088/api/debug?unlock=1",
            {"name": "X-CTF-Token", "value": "sunrise-7"},
            "HTTP 200；响应正文包含 flag_b64 字段",
        ),
    ]
    for sequence, (url, header, observation) in enumerate(steps, 1):
        ...
        repository.save_tool_call(
            ToolCall(
                id=call_id,
                run_id=run.id,
                tool_name="localhost_http_probe",
                tool_id="builtin.localhost_http_probe",
                arguments=arguments,
                input_summary="读取已经公开的本机 CTF 线索",
                result_summary="HTTP 请求完成",
                duration_ms=10,
        ...
        repository.save_execution_step(
            ExecutionStep(
                ...
                preview=(
                    "flag_b64=ZmxhZ3tsb2NhbF9hZ2VudF9mb3VuZF90aGVfZGVidWdfZG9vcn0="
                    if sequence == 4
                    else None
                ),
                decision=(
                    "下一步：Decode base64 flag from /api/status response."
                    if sequence == 4
                    else None
                ),
                artifact_ids=[artifact_id],
        ...
        repository.save_artifact(
            Artifact(
                id=artifact_id,
                thread_id=thread.id,
                filename=f"response-{sequence}.txt",
                kind="http_evidence",
                sha256=f"{sequence:x}" * 64,
                size=100 + sequence,
                mime_type="text/plain",
                storage_ref=f"{thread.id}/response-{sequence}.txt",
            )
        )
    decode_id = uuid4()
    repository.save_tool_call(
        ToolCall(
            ...
            tool_id="ctf.encoding_decode",
            arguments={
                "text": "ZmxhZ3tsb2NhbF9hZ2VudF9mb3VuZF90aGVfZGVidWdfZG9vcn0=",
                "encoding": "base64",
            },
            input_summary="解码 flag_b64",
            result_summary="解码得到 flag{local_agent_found_the_debug_door}",
            duration_ms=5,
            status=CallStatus.SUCCEEDED,
        )
    )
    repository.save_execution_step(
        ExecutionStep(
            ...
            goal="解码 flag_b64",
            action_kind="tool_call",
            action_summary="Decode base64 flag from /api/status response.",
            tool_id="ctf.encoding_decode",
            tool_name="常见编码解码",
            arguments={
                "text": "ZmxhZ3tsb2NhbF9hZ2VudF9mb3VuZF90aGVfZGVidWdfZG9vcn0=",
            ...
            observation_status="success",
            observation_summary="解码得到 flag{local_agent_found_the_debug_door}",
            preview="flag{local_agent_found_the_debug_door}",
            duration_ms=5,
        )
    )
    ...
    markdown, data = ReportGenerator().generate(
        ...
        [],
        {
            "validation_status": "unverified",
            "trace": trace,
            "final_answer": "flag{local_agent_found_the_debug_door}",
        },
    )

    assert len(trace["artifacts"]) == 4
    assert [item["filename"] for item in trace["artifacts"]] == [
        f"response-{item}.txt" for item in range(1, 5)
    ]
...
def test_public_arguments_keeps_only_explicit_ctf_header_values() -> None:
    assert (
        public_arguments({"ctf_header": {"name": "X-CTF-Token", "value": "sunrise-7"}})[
            "ctf_header"
        ]["value"]
        == "sunrise-7"
    )
    assert (
        public_arguments({"ctf_header": {"name": "Authorization", "value": "Bearer private"}})[
            "ctf_header"
        ]["value"]
        == "[REDACTED]"
    )
