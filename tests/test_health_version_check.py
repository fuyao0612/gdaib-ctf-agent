...
def test_health_version_accepts_matching_healthy_payload() -> None:
    validate_health({"status": "ok", "version": "0.4.2"}, "0.4.2")


@pytest.mark.parametrize(
    ("payload", "message"),
    [
        ({"status": "degraded", "version": "0.4.2"}, "状态异常"),
...
def test_health_version_rejects_invalid_payload(
    ...
    with pytest.raises(ValueError, match=message):
        validate_health(payload, "0.4.2")


def test_health_version_cli_reads_json_from_stdin(
    monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    monkeypatch.setattr(sys, "stdin", io.StringIO('{"status":"ok","version":"0.4.2"}'))

    main()

    assert "v0.4.2" in capsys.readouterr().out
