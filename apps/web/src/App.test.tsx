...
describe('App', () => {
  beforeEach(() => { vi.stubGlobal('EventSource', FakeEventSource); vi.stubGlobal('fetch', vi.fn(async (input: string, init?: RequestInit) => {
    if (input.endsWith('/setup/status')) return Response.json({ configured: true, checks: { database: true, master_key: true, admin: true, provider: true }, version: '0.4.0' })
  ...
  it('creates and selects a competition thread', async () => {
    ...
    fireEvent.click(screen.getByText('创建第一个任务')); await screen.findByRole('option', { name: /默认 Agent/ }); fireEvent.change(screen.getByLabelText('运行模式'), { target: { value: 'competition' } }); fireEvent.click(screen.getByRole('button', { name: '创建' }))
