...
const emptyProfile: AgentProfileInput = {
  ...
  workflow: { preset: 'verified' }, report_template: '# {task}\n\n{observations}', enabled: true, is_default: false,
}
...
export default function AgentProfileCenter({ csrf, providers, onChanged }: { csrf: string; providers: ProviderConfig[]; onChanged: () => Promise<void> }) {
  ...
  return <section className="profile-center" data-testid="agent-profile-center">
    ...
    <form className="settings-form" onSubmit={event => { event.preventDefault(); void save() }}>
      ...
      {(expert || wizardStep === 4) && <fieldset><legend>工作流、人工介入与完成验证</legend><div className="form-grid"><label>完成模式<select value={form.completion_mode} onChange={event => setForm({ ...form, completion_mode: event.target.value as AgentProfileInput['completion_mode'] })}><option value="advisory">建议回答（未经外部验证）</option><option value="structured">结构化输出</option><option value="evidence">证据验证</option></select></label><label>工作流预设<select aria-label="工作流预设" value={form.workflow.preset} onChange={event => setForm({ ...form, workflow: { preset: event.target.value as AgentProfileInput['workflow']['preset'] } })}><option value="direct">直接回答</option><option value="planned">规划后执行</option><option value="verified">规划、验证、失败后重规划</option></select></label></div>{form.completion_mode === 'structured' && <label>JSON Schema<textarea aria-label="JSON Schema" value={schemaText} onChange={event => setSchemaText(event.target.value)} /></label>}<div className="form-grid"><label>普通模式补充<select value={form.intervention_policy.normal_mode} onChange={event => setForm({ ...form, intervention_policy: { ...form.intervention_policy, normal_mode: event.target.value as 'wait' | 'fail' } })}><option value="wait">等待用户</option><option value="fail">明确失败</option></select></label><label>竞赛模式补充<select value={form.intervention_policy.competition_mode} onChange={event => setForm({ ...form, intervention_policy: { ...form.intervention_policy, competition_mode: event.target.value as 'replan' | 'fail' } })}><option value="replan">自主重规划</option><option value="fail">明确失败</option></select></label></div></fieldset>}
