...
export default function AgentProfileCenter({
  ...
  const selected = useMemo(
    () => profiles.find((value) => value.profile_id === editingId),
    [profiles, editingId],
  );

  function edit(value: AgentProfile) {
    setEditingId(value.profile_id);
    setForm(inputFromProfile(value));
    setSchemaText(
      value.validation_policy.json_schema
        ? JSON.stringify(value.validation_policy.json_schema, null, 2)
        : "",
    );
    setWizardStep(1);
    setNotice("");
  ...
  async function showVersions(value: AgentProfile) {
    edit(value);
    setVersions(await api.profileVersions(csrf, value.profile_id));
  }
  ...
  async function copy(value: AgentProfile) {
    await api.copyProfile(csrf, value.profile_id, `${value.name} 副本`);
    await load();
  }
  async function makeDefault(value: AgentProfile) {
    await api.defaultProfile(csrf, value.profile_id);
    await load();
  }
  async function remove(value: AgentProfile) {
    await api.deleteProfile(csrf, value.profile_id);
    await load();
  }
  ...
  return (
    <section className="profile-center" data-testid="agent-profile-center">
      ...
      <div className="provider-table">
        {profiles.map((value) => (
          <article
            className={`provider-row ${value.is_default ? "default" : ""}`}
            key={value.profile_id}
          >
            <div>
              <strong>{value.name}</strong>
              <small>
                v{value.version} · {value.completion_mode} · {value.run_mode}
              </small>
              <small>{value.description || "无说明"}</small>
            </div>
            <div className="provider-flags">
              {value.is_default && <span>默认</span>}
              {!value.enabled && <span>停用</span>}
            </div>
            <div>
              <button onClick={() => edit(value)}>编辑</button>
              <button onClick={() => void showVersions(value)}>版本</button>
              <button onClick={() => void copy(value)}>复制</button>
              <button
                disabled={value.is_default}
                onClick={() => void makeDefault(value)}
              >
                设为默认
              </button>
              <button
            ...
            <div className="form-grid">
                ...
                <select
                  ...
                  onChange={(event) => {
                    ...
                    setForm({
                      ...
                      workflow: {
                        preset: strategy === "direct" ? "direct" : "verified",
                      },
                    });
                  }}
                >
                ...
                >
                  ...
                  {providers
                    .filter(
                      (value) =>
                        value.enabled &&
                        value.id !== form.default_provider_id &&
                        !form.fallback_provider_ids.includes(value.id),
                    )
                    .map((value) => (
                      <option value={value.id} key={value.id}>
                        {value.name}
              ...
              <div className="wide provider-chain">
                {form.fallback_provider_ids.map((id, index) => (
                  <span key={id}>
                    {index + 1}.{" "}
                    {providers.find((value) => value.id === id)?.name ?? id}
                    <button
                      type="button"
                      disabled={index === 0}
                      onClick={() => {
                        ...
                        setForm({ ...form, fallback_provider_ids: ids });
                      }}
                    >
                      上移
                    </button>
          ...
          <fieldset>
            ...
            <label>
              ...
              <textarea
                ...
                onChange={(event) =>
                  setForm({ ...form, user_prompt_template: event.target.value })
                }
              />
            </label>
                ...
                <select
                  ...
                  onChange={(event) => {
                    const mode = event.target
                      .value as AgentProfileInput["completion_mode"];
                    setForm({
                      ...form,
                ...
                <select
                  ...
                  onChange={(event) =>
                    setForm({
                      ...
                      intervention_policy: {
                        ...form.intervention_policy,
                        competition_mode: event.target.value as
                          | "replan"
                          | "fail",
                      },
                    })
          ...
          <fieldset>
            ...
            <label>
              ...
              <textarea
                ...
                onChange={(event) =>
                  setForm({ ...form, report_template: event.target.value })
                }
              />
            </label>
      ...
      {versions.length > 0 && (
        <section className="version-history">
          <h4>版本历史与对比</h4>
          {versions.map((value) => (
            <details key={value.version}>
              <summary>
                v{value.version} · {value.created_at}
                <button onClick={() => void rollback(value.version)}>
                  回滚到此版本
                </button>
              </summary>
