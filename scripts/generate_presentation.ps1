...
function Add-PictureFit($slide, [string]$path, [int]$left, [int]$top, [int]$maxWidth, [int]$maxHeight) {
    ...
    try {
        ...
        $x = $left + [int](($maxWidth - $width) / 2)
        $y = $top + [int](($maxHeight - $height) / 2)
        $slide.Shapes.AddPicture($path, $false, $true, $x, $y, $width, $height) | Out-Null
    }
...
function Add-Slide($deck,[int]$i,[string]$title,[string]$body,[string]$evidence,[string]$image='') {
    ...
    if($image -and (Test-Path $image)){Add-PictureFit $s $image 590 126 315 320}
    $line=$s.Shapes.AddShape(1,60,468,840,2);$line.Fill.ForeColor.RGB=10323028;$line.Fill.Solid();$line.Line.Visible=0
    Add-Text $s ('证据：'+$evidence) 65 485 820 35 11 $false 5526612|Out-Null;Add-Text $s ('御网智元 | 初审作品说明 | '+$i+'/'+$items.Count) 65 525 820 20 10 $false 5526612|Out-Null
}
$desktop=Join-Path $OutputRoot '界面-桌面-20260903.png';$mobile=Join-Path $OutputRoot '界面-移动-20260903.png'
$items=@(
@('御网智元','具备自主决策能力的通用网络安全智能体`r`n初审作品说明 | v0.5.0 | 统一工作台 127.0.0.1:8080','正式赛题方案；技术报告；可运行系统',''),
...
@('自主规划与动态调整','默认 Agent 使用 dynamic + verified：先生成任务理解和计划，再执行工具。`r`n遇到工具失败、证据不足或安全拒绝时，系统重规划并保留原因。','Task Brief；Planner；检查点；重规划回归测试',''),
@('工具协同与扩展','ToolSpec 统一输入输出、风险等级、超时和审计契约。`r`n当前覆盖文件、IOC、时间线、编码、哈希、JWT、PCAP、静态分析和授权 localhost HTTP。','ToolSpec；Registry；Executor；19 个工具契约',''),
...
@('解释、验证与复现','事件流展示每一步决策、工具参数摘要、结果和证据引用。`r`n成功工具调用只代表来源可靠，只有 Evidence/Judge 通过才显示独立验证。','trajectory；report；Evidence；EvaluationScorer',''),
...
@('工程化与供应链','Docker Compose 一键部署，提供健康检查、迁移、诊断、备份和恢复脚本。`r`n后端、前端、工具契约与依赖审计纳入统一质量门禁。','87.37% 覆盖率；106 项前端测试；Docker 健康检查',''),
@('通用评测口径','evaluation_cases 提供可扩展的本地评测集合。`r`n每个任务检查结构化结果、成功工具、证据来源、授权范围和预算，不将模型自述当作成绩。','EvaluationRunner；EvaluationScorer；JWT/IOC/编码验收',''),
@('能力边界与提交','仅处理明确授权的附件和本地目标；不提供任意 Shell，不攻击公网目标，不代替外部平台提交。`r`n决赛平台开放后再适配指定网关和判分协议。','技术报告；部署/用户手册；证据矩阵','')
)
$ppt=New-Object -ComObject PowerPoint.Application
try{$deck=$ppt.Presentations.Add();for($i=0;$i -lt $items.Count;$i++){Add-Slide $deck ($i+1) $items[$i][0] $items[$i][1] $items[$i][2] $items[$i][3]};$pptx=Join-Path $OutputRoot '御网智元-方案介绍.pptx';$pdf=Join-Path $OutputRoot '御网智元-方案介绍.pdf';$deck.SaveAs($pptx,24);$deck.SaveAs($pdf,32);$deck.Close()}finally{$ppt.Quit()}
