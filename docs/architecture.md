...
- `apps/api/routes/messages.py` 调用 `yuwang.dispatch.route_message()`；只有明确受控执行意图才创建 Run，停止短语则终止当前 Run。
...
- `apps/web/src/hooks/useChatActions.ts` 使用统一 fetch-SSE；`useWorkbenchData.ts` 只管理 Run 的 EventSource 与持久化恢复。
