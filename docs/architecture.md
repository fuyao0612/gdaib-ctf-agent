...
`Event` 包含 UUID、Run UUID、严格递增 `sequence`、`schema_version`、类型、UTC 时间、公开摘要和脱敏 payload。数据库唯一约束 `(run_id, sequence)`。SSE 使用 `id: sequence`；浏览器自动发送 `Last-Event-ID`，查询 API 也支持 `after`，因此刷新和断线不会重复。超长工具输出会落盘为受控 Artifact，事件、工具调用和检查点只保存摘要与 Artifact ID，不把正文复制进事件流。
...
执行状态、验证状态和证据等级分开保存。`Run.status` 只描述生命周期；图内部的
`completion_ready` 只决定能否收尾，绝不等同于验证通过。没有配置确定性规则时，Agent 可以结束并保留结论，但
`validation_status` 必须是 `unverified`，页面显示“未外部验证”；JSON Schema 校验通过但未获得外部证据时为
`partial`；规则不匹配、候选来源不一致或结构校验失败时为 `failed`。只有 `validated` 才能表述为验证通过。`SuccessVerifier` 只有在
...
`DefaultContextBuilder` 从持久化 TaskSpec、Task Brief、计划、观察、消息、记忆和 Artifact 引用构造
`task_context`。其中固定包含任务摘要、最新用户目标或纠偏、原始约束、已完成步骤、阻塞点、关键决策和
Artifact 引用。运行中补充始终以 `latest_user_instruction_untrusted` 单独放入上下文，优先于滚动摘要；
因此旧摘要不能覆盖最新约束。消息超出窗口时会生成带消息 UUID 范围的可审计摘要，检查点保存所有运行中
补充和附件引用，暂停、刷新和进程恢复均可从同一状态继续。

小型文本附件可在不可信层内受限提供；大型文本附件只提供有界摘要和引用。工具输出超过固定阈值时会创建
`tool_output` Artifact，观察记录改为引用。Artifact 的正文不会成为系统提示、策略或权限来源。
