...
4. 阅读 `apps/web/src/App.tsx`，再看 `MessageComposer.tsx`、`RunViews.tsx` 和 `SettingsCenter.tsx`。
...
5. 前端拿到 Run 后调用 `connect()` 建立 EventSource，并在结束事件到达后重新读取 Thread 详情、审计、记忆和报告。
...
6. `generate_report`：保存助手消息、Markdown/JSON 报告、最终事件和运行摘要。
...
4. `App.connect()` 使用一个 EventSource 处理所有领域事件。断线时浏览器自动带上最后事件 ID，结束后前端重新读取持久化详情。
...
3. Agent 核心：`domain/models.py → agent/state.py → nodes.py → runner.py → engine.py`。
