...
5. 先读 `apps/api/routes/messages.py` 与 `apps/api/run_interactions.py`，再进入
   `apps/api/context.py`，理解同一输入如何调度后台运行或恢复检查点。
...
4. API 使用 `ApiContext.schedule()` 启动后台协程或恢复协程。HTTP 请求不用等待模型完成，因此页面
   可以马上订阅进度。
5. 前端收到 `execution_started` 或交互确认后更新当前 Run；`useWorkbenchData.connect()` 只为活跃
   Run 建立 EventSource，并在结束事件到达后重新读取 Thread 详情、审计、记忆和报告。
...
5. `verify / replan / request_input`：按 advisory、structured 或 evidence 模式完成验证；没有确定性
   外部规则时可以完成回答，但必须标记为 `unverified`。失败时重规划，缺信息时按 Profile 等待或失败。
...
- 设置中心自动创建短生命周期服务端会话；浏览器只持有 HttpOnly Cookie 和内存中的 CSRF 令牌，不持久化凭据。该流程仅适用于本机工作台，公网部署必须加外部身份保护。
...
2. HTTP 边界：`schemas.py → routes/messages.py → run_interactions.py → routes/runs.py → context.py`。
...
**模型为什么不能自行宣告成功？** 模型输出是不可信候选。结构化模式先过 Schema；要显示外部
验证成功，证据模式还必须有真实成功工具证据、来源值绑定和具体的确定性规则。没有这些条件时，
界面只能显示“未外部验证”。
...
**怎样证明可以生产部署？** 就绪探针同时检查数据库、主密钥，以及默认 Agent 能解析到已启用且真实测试成功的 Provider；CI、Docker 冒烟、备份恢复和浏览器 E2E 构成分层证据。
