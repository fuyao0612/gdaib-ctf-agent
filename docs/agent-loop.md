...
本页描述统一消息入口使用的五阶段循环。没有活动 Run 时，每条用户输入都会保存来源消息、固化
TaskSpec 与配置快照并创建受控 Run；Agent 自行决定是否调用已授权工具、请求补充或直接完成。
...
新建的“默认安全 Agent”使用 `dynamic` 策略和 `verified` 工作流：`ingest -> create_task_brief ->
normalize_task -> plan -> select_action`，随后按 `policy_check -> execute_tool -> observe -> replan`
循环推进。这样首次任务会展示公开任务理解、计划、失败后的路径调整和验证阶段；模型不需要工具时仍
可以在一次计划后直接进入验证和报告。默认不自动提取“重要事实”，避免把收尾预算消耗在低优先级记忆上。
需要低延迟单轮回答时，可在设置中心主动选择 `fast/direct` 预设。
这不是放宽安全边界：Run 快照、工具白名单、审批、预算、检查点、审计和确定性验证保持不变。
...
- `apps/api/routes/messages.py` 先处理停止、运行中指引、等待补充和等待澄清；没有活动 Run 时
  不做关键词意图分派，直接创建受控 Run。
